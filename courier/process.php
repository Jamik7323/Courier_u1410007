...
function addCons(){
    $servedBy=$_SESSION['user_name']; // for taking USER .. 
	...
	$sql = "INSERT INTO tbl_courier (cons_no, ship_name, phone, s_add, rev_name, r_phone, r_add,  type, weight,  qty, book_mode,  mode, pick_date,status, comments, book_date ,servedBy)
			VALUES('$ConsignmentNo', '$Shippername','$Shipperphone', '$Shipperaddress', '$Receivername','$Receiverphone','$Receiveraddress', '$Shiptype',$Weight ,  '$Qnty', '$Bookingmode',  '$Mode', '$Packupdate',  '$status', '$Comments', NOW(),'$servedBy')";	
	//echo $sql;
	dbQuery($sql);
...
{
	$cid = (int)$_GET['cid'];
	$sql = "UPDATE  tbl_courier SET history ='Deleted'
			WHERE cid= $cid";
	dbQuery($sql);
